...
#参数介绍
#加载数据的参数
tf.flags.DEFINE_string('raw_file','../text_for_word2vec.txt','data source for training!')
tf.flags.DEFINE_integer('vocab_size',217520,'the size of the data word vocab')

#模型的参数
tf.flags.DEFINE_integer('num_classes',4,'the num of the classes')
...
tf.flags.DEFINE_integer('num_epochs',5,'the eopchs of the training')
...
with tf.Session() as sess:
    ...
    with tf.name_scope('accuracy'):
        ...
        #print('predict',predict,predict.get_shape())
        #print('label',label,label.get_shape())
    ...
    def train_step(x_batch, y_batch):
        feed_dict = {
            ...
            han.max_sentence_length: 20,
            han.batch_size: 64
        }
        _, step, summaries, cost, accuracy ,prediction,= sess.run(
            [train_op, global_step, train_summary_op, loss, acc,predict], feed_dict)
        time_str = str(int(time.time()))
        Label = (np.array([np.argmax(arr) for arr in y_batch]))
        ave_f1_value=f1_score(Label,prediction,average='macro')
        print("{}: step {}, loss {:g}, acc {:g}, ave_f1 {:g}".format(time_str, step, cost, accuracy,ave_f1_value))
    ...
    def dev_step(x_batch, y_batch, writer=None):
        feed_dict = {
            ...
            han.max_sentence_num: 20,
            han.max_sentence_length: 20,
            han.batch_size: len(y_batch)
        }

        step, summaries, cost, accuracy, prediction= sess.run([global_step, dev_summary_op, loss, acc, predict],
                                                                 feed_dict)
        Label = (np.array([np.argmax(arr) for arr in y_batch]))
        ave_f1_value = f1_score(Label, prediction,average='macro')

        time_str = str(int(time.time()))
        print("++++++++++++++++++dev++++++++++++++{}: step {}, loss {:g}, acc {:g}, ave_f1 {:g}".format(time_str, step, cost,
                                                                                           accuracy,ave_f1_value))
